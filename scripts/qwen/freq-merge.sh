...
accelerate launch --config_file static/finetune_config.yaml \
  --main_process_port 29512 mcsmoe/msmoe-merging-qwen.py \
  --model_name="/app/results/mc-smoe/qwen/merge-45e/freq-dom-router-logits-group-freq-merge-32/model.pth" \
  --task="winogrande,arc_challenge,arc_easy,boolq,hellaswag,mmlu,openbookqa,rte" \
  --merge="no" \
  --num_average_groups=45 \
  --output_path="/app/results/mc-smoe/qwen/merge-45e/freq-dom-router-logits-group-freq-merge-32" |& tee results/log_new_qwen_45e_freq-dom-router-logits-group-freq-merge-32
